 ...
 */
 ...
 export const slowAES = {
	...
	aes:{
		...
		{
			let expandedKeySize = (16*(this.numberOfRounds(size)+1));
		...
		galois_multiplication:function(a,b)
		{
			...
			for(let counter = 0; counter < 8; counter++)
			{
				if((b & 1) == 1)
					p ^= a;
				if(p > 0x100) p ^= 0x100;
				let hi_bit_set = (a & 0x80); //keep p 8 bit
				a <<= 1;
		...
		encrypt:function(input,key,size)
		{
			...
			let nbrRounds = this.numberOfRounds(size);
			...
			let expandedKey = this.expandKey(key, size); /* the expanded key */
		...
		decrypt:function(input, key, size)
		{
			...
			for (let i = 0; i < 4; i++) /* iterate over the columns */
				for (let j = 0; j < 4; j++) /* iterate over the rows */
					block[(i+(j*4))] = input[(i*4)+j];
			/* expand the key into an 176, 208, 240 bytes key */
			let expandedKey = this.expandKey(key, size);
	...
	encrypt: function (bytesIn, mode, key, iv)
	{
		...
		if (bytesIn !== null)
		{
			for (let j = 0;j < Math.ceil(bytesIn.length/16); j++)
			{
				let start = j*16;
				let end = j*16+16;
