      ...
      actions: {
        viewGame({commit}){
          if (Object.keys(gameData.data).length >0){
            if (Object.keys(gameData.data.quiz).length >0) {
              ...
              let i = 0;
                    ...
                    imageArray = gameData.data.images;
                    ...
                    commit("setPlayers",Object.values(gameData.top_players).sort(sortBy("points", "desc")));
                    commit('flushMessages', false);
                    commit('team_mode', gameData.data.team_mode);
                    commit('has_game',gameData.data.has_game);
                    commit('has_spin',gameData.data.has_spin);
                    commit('has_video',gameData.data.has_slide);
                    commit('has_mystery_boxes',{'has_mystery_boxes':gameData.data.has_mystery_boxes,'mystery_boxes':gameData.data.mystery_boxes});
                    commit('matchName', gameData.data.title);
                    commit('challenge_id', gameData.data.challenge_id);
                    commit('playerCount', gameData.data.players_count);
                    commit('questions', retVal);
                    commit('set_trivia', gameData.data);
         
               
              // console.log('Decrypted Data:', retVal);
